...
import { CheckCircle2, KeyRound, TicketCheck } from "lucide-react";
...
export default function CheckInLogin() {
  ...
  useEffect(() => {
    async function validarSessaoExistente() {
      ...
      const tokenSalvo = localStorage.getItem("checkin-token");

      if (tokenAdmin) {
        navegar("/check-in", { replace: true });
        return;
      }
      ...
      try {
        const chaveValida = await validarCheckIn(tokenSalvo);
  ...
  async function validarAcesso(evento?: FormEvent) {
    ...
    try {
      const chaveValida = await validarCheckIn(token);

      if (!chaveValida) {
        setMensagemErro("Chave de check-in inválida.");
        setValidando(false);
        return;
      }

      localStorage.setItem("checkin-token", token);
      navegar("/check-in", { replace: true });
    } catch {
        ...
        <div className="mx-auto w-full max-w-6xl grid gap-5 lg:grid-cols-[1fr_0.95fr]">
          <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 backdrop-blur-md p-6 md:p-8 shadow-md">
            ...
            <p className="mt-3 max-w-xl text-gray-300 leading-relaxed">
              Área restrita para consultar participantes inscritos por nome durante o
              atendimento do festival.
            </p>
          </section>
          ...
          <section className="rounded-2xl border border-zinc-700 bg-zinc-950/78 backdrop-blur-xl p-6 md:p-7 shadow-2xl">
            ...
            <form onSubmit={validarAcesso} className="mt-5 flex flex-col gap-4">
              <label className="text-sm text-gray-300">Chave de check-in</label>
              ...
              <div className="pt-3 border-t border-zinc-800 text-xs text-gray-400">
                <p className="inline-flex items-center gap-2">
                  ...
                  Administradores autenticados também podem acessar esta área.
                </p>
              </div>
            </form>
          </section>
